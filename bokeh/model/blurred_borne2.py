...
from .modules import FocusGenerator, SPDC2
from .modules import ConvBlock

class BlurredBorne2(Module):
    def __init__(self, img_channels=3, hidden_channels=[32, 64, 128, 256, 384]):
        ...
        self.stem = Sequential(ConvBlock(img_channels, img_channels*4, kernel_size=7, padding=3),
                               SPDC2(img_channels*4, img_channels*8),

                               ConvBlock(img_channels*4, hidden_channels[0], kernel_size=7, padding=3),
                               SPDC2(hidden_channels[0], hidden_channels[0]*2))
        ...
        # spdc
        self.in_2 = Sequential(SPDC2(in_channels=hidden_channels[1], hidden_channels=hidden_channels[1]*2),
                               SPDC2(in_channels=hidden_channels[1], hidden_channels=hidden_channels[1]*2))

        self.in_4 = Sequential(SPDC2(in_channels=hidden_channels[2], hidden_channels=hidden_channels[2]*2),
                               SPDC2(in_channels=hidden_channels[2], hidden_channels=hidden_channels[2]*2))
        #  TODO
        self.bot = Sequential(ConvBlock(in_channels=hidden_channels[3], out_channels=hidden_channels[4], kernel_size=3, padding=1),
                              SPDC2(in_channels=hidden_channels[4], hidden_channels=hidden_channels[4]*2),
                              SPDC2(in_channels=hidden_channels[4], hidden_channels=hidden_channels[4]*2))

        self.out_4 = Sequential(SPDC2(in_channels=hidden_channels[2] + hidden_channels[4]//4,
                                     hidden_channels=hidden_channels[2]*2 + hidden_channels[4]//2, has_skip_connection=False),
                                SPDC2(in_channels=hidden_channels[2] + hidden_channels[4]//4,
                                     hidden_channels=hidden_channels[2]*2 + hidden_channels[4]//2))

        self.out_2 = Sequential(SPDC2(in_channels=hidden_channels[1] + hidden_channels[2]//4 + hidden_channels[4]//16,
                                     hidden_channels=hidden_channels[1]*2 + hidden_channels[2]//2 + hidden_channels[4]//8,
                                     has_skip_connection=False),
                                SPDC2(in_channels=hidden_channels[1] + hidden_channels[2]//4 + hidden_channels[4]//16,
                                     hidden_channels=hidden_channels[1]*2 + hidden_channels[2]//2 + hidden_channels[4]//8))
        ...
        self.out_conv = Sequential(SPDC2(in_channels=6, hidden_channels=6*3),
