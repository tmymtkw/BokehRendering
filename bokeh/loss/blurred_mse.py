...
class BlurredMSELoss(MSELoss):
    def __init__(self, kernel_size=[5, 9, 11], sigma=[1, 1.25, 1.5], device="cpu"):
        super().__init__()
    ...
    def forward(self, img_output, img_target, num):
        # resize target
        # check
        ...
        if not self.is_safe:
            ...
            for kernel in self.kernels:
                assert kernel.device == img_target.device, f"[ERROR] incorrect device : {kernel.device} and {img_target.device}"
            self.is_safe = True
        ...
        for kernel, padding in zip(self.kernels, self.paddings):
            blur_target =  blur_target + conv2d(resized_target, kernel, padding=padding, groups=3) / bit
            bit /= 2
